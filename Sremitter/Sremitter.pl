...
sub RMTR_main
{
 ...
 #проверка на _действительный_ персональный платежный код
 $csum=0;
 $csum+=$_ foreach split //,$RMTR_post1id;
 $csum%=10; # _действительная_ контрольная сумма посчитанная по переданному номеру из формы
 
 #получаем контрольную сумму, которая была передана из формы
 $csum_post=substr($RMTR_post1,-1);
 
 #защита от переводов на несуществующий персональный платежный код получателя
 &Error('Вы указали несуществующий персональный платежный код получателя.') if $csum != $csum_post; 
 ...
 #добавим к сумме перевода стоимость услуги
 ...
 if ($RMTR_priceprc > 0)
   {
     ...
     #добавим к сумме перевода процент от переводимой суммы
 ...
 &Error('Вы указали сумму больше, чем у Вас есть на счете.') if ($U{$Mid}{final_balance} - $RMTR_post2 - $RMTR_price - $RMTR_prc_tmp) < 0;
 ...
 if ($RMTR_price > 0)
   {
     ...
     $sql="INSERT INTO pays (mid,cash,type,bonus,category,admin_id,admin_ip,reason,coment,time) ".
         "VALUES($Mid,-$RMTR_price,10,'y',105,$Adm{id},INET_ATON('$RealIp'),'За услугу Поделиться балансом','За услугу Поделиться балансом',$t)";
 ...
 if ($RMTR_priceprc > 0)
   {
     ...
     $sql="INSERT INTO pays (mid,cash,type,bonus,category,admin_id,admin_ip,reason,coment,time) ".
         "VALUES($Mid,-$RMTR_prc_tmp,10,'y',105,$Adm{id},INET_ATON('$RealIp'),'За услугу Поделиться балансом','За услугу Поделиться балансом',$t)";
 ...
 $rows=&sql_do($dbh,"UPDATE users SET balance=balance-".$RMTR_post2_tmp." WHERE id=$Mid LIMIT 1");
 if ($rows<1)
   {
 ...
 $rows=&sql_do($dbh,"UPDATE users SET balance=balance+".$RMTR_post2." WHERE id=$RMTR_post1id LIMIT 1");
 if ($rows<1)
   {
    &ToLog("! mid $Mid использовал услугу Поделиться балансом $RMTR_post2 $gr для ППК $RMTR_post1, но после внесения платежа в таблицу платежей произошла ошибка изменения баланса клиента");
    &Error("Произошла ошибка перевода суммы. Обратитесь к администрации.",$EOUT);
 ...
 &OkMess(&bold_br("Счет с персональным платёжным кодом $RMTR_post1 пополнен на $RMTR_post2 $gr").'Если текущих средств достаточно для включения доступа в интернет - доступ получателя будет включен в течение нескольких минут.'.$br2);
